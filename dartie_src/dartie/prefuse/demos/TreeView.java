...
public class TreeView extends Display {
    ...
    public static JComponent demo(final String label) {
        ...
        Tree t = null;
        try {
/*        	IParser parser = ParserFactory.createParser("zhongyijichu.rdfs");
            t = (Tree)new TreeMLReader().readGraph(parser.getClassStream());    */
            IParser parser = ParserFactory.createParser("zhongyijichu.rdfs", "");
            t = (Tree)new TreeMLReader().readGraph(parser.getClassStream());
            //t = new Tree();
        } catch ( Exception e ) {
        ...
        NewJSearchPanel search = new NewJSearchPanel(tview.getVisualization(),
            treeNodes, Visualization.SEARCH_ITEMS, label, true, true);
        search.setShowResultCount(true);
        search.setBorder(BorderFactory.createEmptyBorder(3,0,0,0));
        search.setFont(FontLib.getFont("simsun", Font.PLAIN, 12));
        search.setFocusControl(tview.focusControl);
