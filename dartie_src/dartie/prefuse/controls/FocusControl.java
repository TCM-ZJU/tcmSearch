...
import dartie.application.GraphView;
import dartie.application.RadialGraphView;
    ...
    public void itemClicked(VisualItem item, MouseEvent e) {
        if(UILib.isButtonPressed(e, button) &&
        	...
        	if (nodetype.equals("class")) {
        		GraphView.GetGraph(uri);
        	} else if (nodetype.equals("instance")) {
        		RadialGraphView.GetGraph(uri);
        	}
        	System.out.println(nodetype);
        	System.out.println(uri);
        	return;
        }
        String uri = item.getString("uri").replaceAll("#","__");
        try {
			uri = URLEncoder.encode(uri,"UTF-8");
		} catch (UnsupportedEncodingException e1) {
			e1.printStackTrace();
		}
        String url = "ontologyInfo.htm?nodeIdentity="+uri+"&ontoIdentity="+uri+"&expanding=true";
    	jsObject.eval ("window.opener.location.href='"+url+"';");
        //jsObject.eval ("alert('"+url+"');");
    	if ( UILib.isButtonPressed(e, button) &&
