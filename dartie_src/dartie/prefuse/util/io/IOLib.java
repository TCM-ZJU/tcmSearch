package dartie.prefuse.util.io;
...
import dartie.prefuse.data.io.CSVTableReader;
import dartie.prefuse.data.io.DelimitedTextTableReader;
import dartie.prefuse.data.io.GraphMLReader;
import dartie.prefuse.data.io.GraphReader;
import dartie.prefuse.data.io.TableReader;
import dartie.prefuse.data.io.TreeMLReader;
import dartie.prefuse.util.StringLib;
import dartie.prefuse.util.collections.ByteArrayList;
...
public class IOLib {
    ...
    public static Graph getTreeFile(Component c) {
        JFileChooser jfc = new JFileChooser();
        jfc.setDialogType(JFileChooser.OPEN_DIALOG);
        jfc.setDialogTitle("Open Tree File");
        jfc.setAcceptAllFileFilterUsed(false);
        ...
        // TreeML
        ff = new SimpleFileFilter("rdfs",
                "Tree File (*.rdfs, *.owl)",
                new TreeMLReader());
        ff.addExtension("rdfs");
        ff.addExtension("owl");
        jfc.setFileFilter(ff);
        ...
        try {
        	ParserFactory.SetInputFile(f.getAbsolutePath());
        	IParser parser = ParserFactory.createParser(f.getAbsolutePath(), "");
            return (new TreeMLReader().readGraph(parser.getClassStream()));
        } catch ( Exception e ) {
            Logger.getLogger(IOLib.class.getName()).warning(
                e.getMessage() + "\n" + StringLib.getStackTrace(e));
            return null;
